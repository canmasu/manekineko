...
if (isset($_POST['wish']) && !empty($_POST['wish'])) {
    ...
    $wish = '幸運を祈る！運がよくなる宇宙からのサイン';
    //虎年祝你财源滚滚，发得象肥虎，身体壮的象头虎，爱情甜的象奶虎，好运多的象虎毛，事业蒸蒸象大虎
    //$wish = 'Congratulation on the business grand opening. Stay wealthy, healthy and prosperity';
}
...
$NFT -> day             = gmdate("w",(int)substr($energies, 49,11));
...
$waveHand = ' <g id="wave">
<path d="M971.5,586.5s28.66-75.58,31-83c6-19,3-26,3-26l-6-10c-1.77-2.94-5.76-6.41-10-5a32.18,32.18,0,0,0-7,4s-8-4-10,10-23.67,87.4-23.67,87.4" style="fill: #fff;stroke: #000;stroke-miterlimit: 10;stroke-width: 3px"/>
<path d="M982.65,475.23a1.93,1.93,0,0,0-3-1.46c-2.64,1.18-6.14,18.73-6.14,18.73s-3.43,12,9.16,14.49c10.15,2,13.73-17.24,13.32-24.11-.19-3.17-1.37-9-5.59-11.38C988.05,470.21,984.38,471.53,982.65,475.23Z" style="fill: #f9c6b7"/>
</g>';


// Base Display wish message and the DAY
$wishDisplay = file_get_contents('template/base/'.$NFT -> day.'.txt');
...
//if(file_exists('images/'.$NFT -> id.'.svg')==0){
    // WRITE ARTWORK 1
    $newFileName = 'images/'.$NFT -> id.'.svg';
    $newNeko = fopen($newFileName, "w") or die("Unable to open file!");
    fwrite($newNeko, $meow);
    fwrite($newNeko, $meow_footer);
    fclose($newNeko);


    // WRITE ARTWORK 2
    $newFileName = 'images/'.$NFT -> id.'b.svg';
    $newNeko = fopen($newFileName, "w") or die("Unable to open file!");
    fwrite($newNeko, $meow);
    fwrite($newNeko, $waveHand);
    fwrite($newNeko, $meow_footer);
    fclose($newNeko);

    // WRITE ARTWORK 3
    $newFileName = 'images/'.$NFT -> id.'c.svg';
    $newNeko = fopen($newFileName, "w") or die("Unable to open file!");
    fwrite($newNeko, $meow);
    fwrite($newNeko, $wishDisplay);
    fwrite($newNeko, $meow_footer);
    fclose($newNeko);


    // WRITE ARTWORK 4
    $newFileName = 'images/'.$NFT -> id.'d.svg';
    $newNeko = fopen($newFileName, "w") or die("Unable to open file!");
    fwrite($newNeko, $meow);
    fwrite($newNeko, $waveHand);
    fwrite($newNeko, $wishDisplay);
...
// RANDER INTO PNG FORMAT
$im = new Imagick();


// FIRST PNG
    $draw = new ImagickDraw();
    $svg = file_get_contents('images/'.$NFT -> id.'.svg');
    ...
    $lineHeight = 1400;
    $im->annotateImage($draw, 1450 , $lineHeight, 0, $NFT -> name);

    $im->setImageFormat("png24");
    $im->writeImage('images/png/'.$NFT -> id.'.png');
    $im->clear();
    $im->destroy();


echo 'images/'.$NFT -> id.'b.svg';

// SECOND PNG
    $draw = new ImagickDraw();
    $svg2 = file_get_contents('images/'.$NFT -> id.'b.svg');
    $im->readImageBlob($svg2);
    ...
    $im->setImageFormat("png24");
    $im->writeImage("images/png/".$NFT -> id."b.png");
    $im->clear();
    $im->destroy();
...
// THIRD PNG
    $draw = new ImagickDraw();
    $svg3 = file_get_contents('images/'.$NFT -> id.'c.svg');
    $im->readImageBlob($svg3);
    ...
    $draw->setFillColor('#ff3c00');

    // Wrap lines limited to 12 characters and break
    // them into an array
    $lines = explode("\n", wordwrap($wish, 42, "\n", true));

    echo 'wish length '.count($lines);
    ...
    foreach ($lines as $text) {
        $im->annotateImage($draw, 350 , $lineHeight, 0, $text);
        $lineHeight += 45;
    }

    $im->setImageFormat("png24");
    $im->writeImage("images/png/".$NFT -> id."c.png");
    $im->clear();
    $im->destroy();
...
// SECOND PNG
    ...
    if (strlen($wish) != strlen(utf8_decode($wish))) {
        $characterLimit = 60;
        // require factor of 3
    } else {
        $characterLimit = 47;
...
// FOURTH PNG
    $draw = new ImagickDraw();
    $svg3 = file_get_contents('images/'.$NFT -> id.'d.svg');
    $im->readImageBlob($svg3);
    ...
    $im->setImageFormat("png24");
    $im->writeImage("images/png/".$NFT -> id."d.png");
    $im->clear();
    $im->destroy();



// RANDER INTO GIF FORMAT
require "src/AnimGif.php";
$anim = new GifCreator\AnimGif();
$frames = array(
    "images/png/".$NFT -> id.".png",                          // image file path
    "images/png/".$NFT -> id."b.png",                          // image file path
    "images/png/".$NFT -> id.".png",                          // image file path
    "images/png/".$NFT -> id."b.png",                          // image file path
    "images/png/".$NFT -> id.".png",                          // image file path
    "images/png/".$NFT -> id."b.png",                          // image file path
    ...
    "images/png/".$NFT -> id."b.png",                          // image file path
    "images/png/".$NFT -> id.".png",                          // image file path
    "images/png/".$NFT -> id."c.png",                          // image file path
    "images/png/".$NFT -> id."d.png",                          // image file path
    "images/png/".$NFT -> id."c.png",                          // image file path
    "images/png/".$NFT -> id."d.png",                          // image file path
    "images/png/".$NFT -> id."c.png",                          // image file path
    "images/png/".$NFT -> id."d.png",                          // image file path
...
$durations = array(20, 30,10,20,30,100,20,30, 30,10,20,50);
$anim = new GifCreator\AnimGif();
$anim->create($frames, $durations);
$anim->save("images/gif/".$NFT -> id.".gif");
...
Array (
    ...
    Array (
        ...
        "3" => Array (
            "trait_type" => "縁起物 Lucky item",
            "value" => $NFT ->luckyItem,
        ),
        "4" => Array (
            "trait_type" => "神の力 Maneki Power",
            "value" => $NFT -> manekiPower.' ← '.number_format((int)$NFT -> basePower),
        ),
        "5" => Array (
            "trait_type" => "豊か Wealth",
            "value" => number_format((int)$NFT -> wealth),
        ),
        "6" => Array (
            "trait_type" => "知恵 Wisdom",
            "value" => number_format((int)$NFT -> wisdom),
        ),
        "7" => Array (
            "trait_type" => "関係 Relationship",
            "value" => number_format((int)$NFT -> relationship),
        ),
        "8" => Array (
            "trait_type" => "元気 Health",
            "value" => number_format((int)$NFT -> health),
        ),
        "9" => Array (
            "trait_type" => "機会 Opportunity",
            "value" => number_format((int)$NFT -> opportunity),
        ),
        "10" => Array (
            "display_type" => "date",
            "trait_type" => "お誕生日 Birthdate",
            "value" => $NFT -> birthdate,
        ),
        "11" => Array (
            "trait_type" => "顔 Face",
        ...
        "15" => Array (
            "trait_type" => "天気 Weather",
            "value" => $NFT -> weather,
        )
    ),
    "description" => $wish,
    "external_url" => "https://nft.neko.exchange/",
    "image" => 'https://nft.neko.exchange/gif/'.$NFT -> id .'.gif',
