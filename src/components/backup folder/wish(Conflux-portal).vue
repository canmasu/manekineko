    ...
    <div class="mint_container">
        ...
        <el-form :model="form"  v-if="display.isGuardian" :rules="wishRules" ref="guardianForm">
            ...
            <el-form-item class="dialog-footer">
                ...
                <el-button type="primary" @click="updateData()">Update Data</el-button>
                <el-button type="primary" @click="getAccounts()">getAccounts</el-button>
                <el-button type="primary" @click="ConfluxPortalInstall()">ConfluxPortalInstall</el-button>
    ...
    if (typeof window.conflux !== "undefined"){
        ...
        try {
            ...
            contract_payment.paymentByCoin('9000001000112345678901',this.newNFT.guardian).sendTransaction({
                ...
                /** 
                const TxHash = contract_payment.getTransactionReceipt(receipt.transactionHash );
                contract_payment.abi.decodeLog(TxHash.logs[1])

                const event = contract_payment.abi.decodeLog(receipt.logs[0])
                console.log("purchase event:", event)
                */
                console.log("event log PAYMENT:", receipt.logs[14])
                const paymentlog = contract_payment.abi.decodeLog(receipt.logs[1])

                console.log("purchase receipt", receipt)
                console.log("Payment Event", paymentlog)
    ...
    async payByCoins (_machine,_refNekoId){
        ...
        this.$message.success('Start Minting');
            ...
            contract_payment.paymentByCoin(_machine,_refNekoId).sendTransaction({
                ...
                //const event = contract_payment.abi.decodeLog(receipt.logs[0])
               // console.log("Receipt log:", contract_payment.abi.decodeLog(receipt[14].logs))
                console.log("transactionHash:", receipt.transactionHash )

                //this.generateNewNFT(receipt.events.PAYMENT.returnValues._nftID, receipt.transactionHash );
                //console.log('pay with coin',receipt);
                //console.log('New NFT ID : ',receipt.events.PAYMENT.returnValues._nftID);
