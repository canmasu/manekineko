    ...
    <div class="mint_container">
        ...
        <el-form :model="form"  v-if="display.isGuardian" :rules="wishRules" ref="guardianForm">
            ...
            <el-form-item class="dialog-footer">
                ...
                <el-button type="primary" @click="updateButton()">Update Wallet</el-button>
...
//import '../web3/conflux-portal-onboarding.bundle'
...
const {Conflux : ConfluxJS, Conflux, Drip } = ConfluxJSSDK
...
console.log('confluxJS version :', confluxJS.version);
...
//const Sohem = confluxJS.wallet.addPrivateKey('0xED7D29E2C84D7588421F17A88FF2CB0D4CB86F8D2A03193B9E00CF9294A3EC27', 1);



//const Sohem = conflux.wallet.addPrivateKey('0xAA0CACF21CFFB7CBD53CA4765294475C444F743F760C313F0742AC99FBF1E734', 1);

           // const account = conflux.wallet.addPrivateKey(process.env.PRIVATE_KEY, 1);
          //  console.log('Connect Private key :', process.env.PRIVATE_KEY);


//console.log('sohem :', Sohem);

//connect Collectibles Contract 
const contract_collectibles =  confluxJS.Contract({abi:abi_collectible, address:'cfxtest:acbz06p71gzn1e00c3j06gwgnebvgdw1cj50pg1teh'});
const contract_collectibles_addr = 'cfxtest:acbz06p71gzn1e00c3j06gwgnebvgdw1cj50pg1teh';
//connect Exchange Contract
const contract_payment = confluxJS.Contract({abi:abi_payment, address:'cfxtest:ace4szmrknum7e9m0ykjy9ghhgpkygy48paem6zevb'});
//connect Neko Contract
const contract_neko = confluxJS.Contract({abi:abi_neko, address:'cfxtest:acfc7ss6kzp1cga5f342wa9r8m71bkxczpfrapgn9d'});
...
export default {
  ...
  mounted() {
        ...
        console.log('update button',onboarding);
        
    if (typeof window.conflux !== "undefined") {
        this.isConfluxPortalConnected();
  ...
  methods:{
    async quickMint(){
        ...
        let contract_pay = confluxJS.Contract({abi:abi_payment, address:'cfxtest:ace4szmrknum7e9m0ykjy9ghhgpkygy48paem6zevb'});
    ...
    if (typeof window.conflux !== "undefined"){
        ...
        try {
            ...
            contract_pay.paymentByCoin('9000001000112345678901',this.newNFT.guardian).sendTransaction({
                from: this.account,
                gas: '0x493E0', //300,000
                storageLimit: '0x2710', //300
                gasPrice: '0xBB8', //3000
      ...
      conflux.provider.sendAsync(
        ...
        (err, result) => {
          if (err) {
            ...
            console.log('personal sign :', JSON.stringify(result));
          }
        }
      )
    },
    updateConnection(){
        if (this.isConfluxPortalInstalled()) {
            conflux.autoRefreshOnNetworkChange = false
            conflux.on('networkChanged')
            conflux.on('chainIdChanged')
            conflux.on('accountsChanged').then((newAccounts) => {

            const connecting = Boolean(
                (!this.accounts || !this.accounts.length) && newAccounts && newAccounts.length
            )
    ...
    isConfluxPortalConnected(){
        console.log('isConfluxPortalConnected :', this.accounts);
        return this.accounts && this.accounts.length > 0
    },
    isLikeBase32Address(addr) {
    ...
    setAddressType(addr) {
        ...
        if (this.isLikeBase32Address(addr)) {
            this.USE_BASE32_ADDRESS = true
        } else {
            this.USE_BASE32_ADDRESS = false
        }
    },
    mintConfirm() {
    ...
    async getCurrentAccounts(){
        try {
            ...
            console.log ('Current Account :', accounts[0] || 'Not able to get accounts')
        } catch (error) {
            console.error(error)
        }
    ...
    async ConnectWallet(){
        ...
        if (typeof window.conflux !== "undefined"){
            ...
            try {
                ...
                console.log('Connect Accounts JS:', this.account);
