<template>
    ...
    <div class="mint_container">
        ...
        <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="approvePayment(payment.tokens[form.payment].tokenAddress, payment.tokens[form.payment].amount)">Approve</el-button>
        </div>
        </el-dialog>
...
const contract_payment = conflux.Contract({address:'cfxtest:ace4szmrknum7e9m0ykjy9ghhgpkygy48paem6zevb',abi:abi_payment});
...
export default {
  ...
  methods:{
        ...
        if (this.form.guardian!='guardian'){
            ...
            if (valid) {
                ...
                if(index==0){
                    ...
                    console.log('Pay with ERC20 :', this.payment.tokenSymbol)
    ...
    async approvePayment (){
        ...
        console.log('Approve ERC20 To spend',_machine, ', ',_refNekoId, ', ', this.account);
        console.log('ERC Qty',this.payment.tokens[0].tokenQuantity);

        
        await contract_neko.approve(contract_payment , this.payment.tokens[0].tokenQuantity).call({
        from: this.account,
        }).then((res) => {
