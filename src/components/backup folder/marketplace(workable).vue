                ...
                <li class="item"  v-for="(item) in NFTs" :key="item.id" >
                    ...
                    <div class="item-wrap">
                        ...
                        <ul>
                            ...
                            <li>Payment Token : {{item.paymentToken}}</li>
                            <li>Start Price : {{item.startingPrice}}</li>
                            <li>End Price : {{item.endingPrice}}</li>
                            <li>duration : {{item.duration}}</li>
                            <li>currentPrice : {{item.currentPrice}}</li>
                        </ul>
...
import ConfluxPortalOnboarding from 'conflux-portal-onboarding'
    ...
    //const contract_marketplace = conflux.Contract({abi:abi_exchange, address:'cfxtest:acbskvp4y17jg2ykfcc8akdr44ptw2gnpjj685zjnw'});
    const contract_marketplace = conflux.Contract({abi:abi_exchange, address:'cfxtest:acf48ser8g2zwxva2dx1b0xrgucwd6kwv25pkj3zyw'});
...
export default {
    ...
    methods :{
        async getAuctions() {
            ...
            for (let i=59480281; i<latestEpoch ;i+=999){
